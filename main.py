...
def _train(cfg, _run, ex, tensorboard):
    ...
    for task_i in range(inc_dataset.n_tasks):
        ...
        ex.logger.info(f"task: {task_i} avg_acc: {avg_acc}")
    ...
    # 输出自定义统计：avg_acc, avg_acc_mean, last_acc, last_acc_mean（缩进为4）
    try:
        results_dict = {
            "avg_acc": avg_acc,
        ...
        ex.logger.info(json.dumps(results_dict, indent=4))
    except Exception as e:
        ex.logger.info(f"summary compute error: {e}")
